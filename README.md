...
| M4 | **主地圖第一幀與原版 oracle PNG 逐點相同** | **完成**。主地圖客戶區 608×538 只差參考幀上的滑鼠游標 54 px（游標是 Windows 畫的，這一邊不畫）；三個視窗的幾何與參考幀逐項相同，不需任何位移 |
...
那 54 個像素全部集中在 `(397..412, 297..314)`，是**參考幀上的滑鼠游標**
...
還差的是 **Status 面板**：三個客戶區裡只有它對不上（82.7% 不同）。底色走
`sub_5FDC8(rect, 色號)`，而 `sub_28175` 在 256 色模式下建的是 8×8 的圖樣筆刷
不是實心筆刷；圖樣填色補上之後那一片仍是黑的，鏈上還有一環沒找到
（[`docs/spec/006`](docs/spec/006-oracle-comparison.md) §5.4）。
