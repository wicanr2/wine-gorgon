# 直接事件與記憶體寫入追蹤（2026-09-06，Civ1 I542）

新增 `reg` 比較後暫存器注入與 `watchmemuntil` 寫入觀察；不硬編遊戲位址。
`go vet ./...`、`go test ./... -count=1` 通過，新增原子拒絕、高位元保存、
不推進時鐘、最後一步寫入與未命中失敗收據測試。
Civ1 兩種呼叫脈絡各 45 次原版 ZOC 直接呼叫通過，已驗出有效文明與堆疊
殘值的實際判定差異，全程不靠 rand() 觸發事件；詳見 docs/probe-control.md。
基底 c8ab575；本輪僅修改 cmd/nerun/probe* 與上述文件。所有容器 --rm，未建映像。
